...
def add_black_cover(seq, name, channel, start_frame, end_frame, target_width, offset_x, fade_frames, tmp_dir):
    ...
    transform.offset silently fail to composite (the strip's blend_alpha
    reads back as correct, but nothing shows in the render). Image/movie
    strips — the same strip type already used for the left/right videos —
    don't have this problem, so we reuse that proven path here.
    """
    ...
    # The PNG is already exactly target_width x HEIGHT (matches the panel
    # natively), so no scaling is needed — just the same offset formula
    # used to position the video panels.
    black.transform.offset_x = offset_x
    black.transform.offset_y = 0

    # Alpha Over so blend_alpha smoothly composites this strip over
    # whatever is on the channels below.
...
def main():
    ...
    black_strips = []  # keep references for debugging/inspection below
...
# ---------------------------------------------------------------------------
# DEBUG: dump the actual state of each black-cover strip at a frame where
# it should be fully opaque, and render ONE still frame to a PNG so we can
# visually confirm what the compositor is actually producing.
# ---------------------------------------------------------------------------
DEBUG_STILL_FRAME = None
if black_strips:
    first = black_strips[0]
    # pick a frame safely inside the strip's "fully black" plateau
    mid_frame = int((first.frame_final_start + first.frame_final_end) / 2)
    DEBUG_STILL_FRAME = mid_frame

    scene.frame_set(mid_frame)
    bpy.context.view_layer.update()

    print("\n--- DEBUG: black cover strip state ---")
    for b in black_strips:
        print(
            f"{b.name}: channel={b.channel} "
            f"frame_final_start={b.frame_final_start} frame_final_end={b.frame_final_end} "
            f"blend_type={b.blend_type} blend_alpha(at current frame)={b.blend_alpha:.3f} "
            f"transform.scale=({b.transform.scale_x:.3f},{b.transform.scale_y:.3f}) "
            f"transform.offset=({b.transform.offset_x},{b.transform.offset_y}) "
            f"mute={b.mute} channel_muted={scene.sequence_editor.channels[b.channel].mute if b.channel <= len(scene.sequence_editor.channels) else 'n/a'}"
        )
    print(f"Rendering debug still at frame {mid_frame} ...")

    debug_png_path = os.path.join(os.path.dirname(OUTPUT_PATH), "debug_still.png")
    scene.render.filepath = debug_png_path
    scene.render.image_settings.media_type = 'IMAGE'
    scene.render.image_settings.file_format = 'PNG'
    bpy.ops.render.render(write_still=True)
    print(f"Debug still saved to: {debug_png_path}")

    # restore video output settings for the real render
    scene.render.filepath = OUTPUT_PATH
    scene.render.image_settings.media_type = 'VIDEO'
    scene.render.image_settings.file_format = 'FFMPEG'
    scene.render.ffmpeg.format = 'MPEG4'
