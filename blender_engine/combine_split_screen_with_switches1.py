...
def add_black_cover(seq, name, channel, start_frame, end_frame, target_width, offset_x, fade_frames):
    ...
    # Blender 5.x's new_effect() signature is (name, type, channel,
    # frame_start, length, input1, input2) — note "length", not frame_end.
    ...
    # Explicitly use Alpha Over so blend_alpha smoothly composites this
    # strip over whatever is on the channels below. The default blend
    # mode (Replace) does not reliably respect partial opacity for this
    # kind of stacked compositing.
